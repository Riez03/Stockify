            ...
            <x-table.table-layout :title="'List Products Data'" :description="'List Item of Products Information'">
                @slot('additional')
                    <div class="flex justify-between items-center gap-3">
                        <div class="flex justify-start items-start gap-3">
                                ...
                                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data">
                                  @csrf
                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                        ...
                                        <x-form.textarea name="description" label="Description" placeholder="Write Product Description" />
                                        <x-form.input name="purchase_price" label="Purchase Price" placeholder="Rp. 0" type="number" required />
                                        <x-form.input name="selling_price" label="Selling Price" placeholder="Rp. 0" type="number" required />
                            ...
                            <a href="{{ route('attributes.index') }}" role="button" class="flex focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">
                                <x-tabler-arrows-split-2 class="w-6 h-5 me-0 -ms-1 md:me-2" />
                                <span class="hidden md:block">View Attributes</span>
                            </a>
                        </div>
                        <a href="" role="button" class="text-gray-900 bg-white hover:bg-gray-100 border border-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700">
                            <x-tabler-table-export class="w-6 h-5 me-2 -ms-1" />
                            Export
                        </a>
                    </div>
                @endslot
                ...
                @foreach ($products as $product)
                    <tr>
                        ...
                        <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap border-b dark:border-gray-500 dark:text-white">
                          <a href="{{ route('products.edit', $product->id) }}" class="text-white bg-yellow-300 hover:bg-yellow-400 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-yellow-300 dark:hover:bg-yellow-400 dark:focus:ring-yellow-800 hover:cursor-pointer">
                            <x-heroicon-s-pencil-square class="w-5 h-5 text-white" />
                          </a>
                          <a href="{{ route('products.show', $product->id) }}" class="text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-800 dark:focus:ring-blue-800 hover:cursor-pointer">
                            <x-heroicon-s-eye class="w-5 h-5 text-white" />
                          </a>
                          <form action="{{ route('products.destroy', $product->id) }}" method="POST" style="display: inline" onsubmit="return confirm('Are you sure you want to delete this supplier?');">
                            @csrf @method('DELETE')
