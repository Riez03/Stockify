...
@section('content')
    <div class="px-4 pt-6">
        ...
        <section>
            <x-table.table-layout :title="'List Action Stock Transaction'" :description="'History Transaction In/Out Item Products'">
                @slot('additional')
                    <div class="flex justify-between items-center gap-3">
                        <form action="{{ route('stock.index') }}" method="GET" class="flex justify-center gap-2">
                            ...
                            <button type="submit" class="text-gray-900 mt-7 bg-white hover:bg-gray-100 border border-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700">
                                <x-tabler-filter class="w-5 h-5 mr-1" />
                                Filter
                            </button>
                        </form>
                        <a href="" role="button" class="text-gray-900 mt-7 bg-white hover:bg-gray-100 border border-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700">
                ...
                @foreach ($stockByType as $transaction)
                    <tr>
            ...
            <x-table.table-layout :title="'Stock Transaction by Filter'" :description="'Filter Stock Transaction Item Products'">
                        ...
                        <div class="flex justify-between items-center gap-3">
                            ...
                            <x-form.select-option name="categories" label="Category:" placeholder="All Categories">
                                @foreach($category as $categories)
                                    <option value="{{ $categories->id }}" {{ request('categories') == $categories->id ? 'selected' : '' }}>{{ $categories->name }}</option>
                                @endforeach
                            </x-form.select-option>
                            <button type="submit" class="text-gray-900 mt-7 bg-white hover:bg-gray-100 border border-gray-300 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:focus:ring-gray-600 dark:bg-gray-800 dark:border-gray-700 dark:text-white dark:hover:bg-gray-700">
                                <x-tabler-filter class="w-5 h-5 mr-1" />
                ...
                @foreach ($stockByCriteria as $transaction)
                    <tr>
                        <td class="p-4 text-sm font-normal text-gray-900 border-b whitespace-nowrap dark:border-gray-500 dark:text-white">
                            {{ $loop->index + 1 }}</td>
                        <td class="p-4 text-sm font-normal text-gray-900 whitespace-nowrap border-b dark:border-gray-500 dark:text-white">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            var periodSelect = document.querySelector('select[name="periods"]');
